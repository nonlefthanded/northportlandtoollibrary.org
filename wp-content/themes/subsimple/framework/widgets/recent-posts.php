...
class subsimple_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'subsimple_rp', // Base ID
			__('Hanne Recent Posts with Thumbnails', 'subsimple'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'subsimple' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		$title = __('Latest Posts','subsimple');
		$no_of_posts = 5;
	endif;
		...
		<?php
         ...
         ?>
		         ...
		         <div class='rp-date'><?php the_time(__('M j, Y','subsimple')); ?></div>
		         </li>      
		...
		?>
		      ...
		      <?php _e('Oops, there are no posts.','subsimple'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'subsimple' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'subsimple' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','subsimple' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'subsimple' ); ?></label>
	...
	public function update( $new_instance, $old_instance ) {
		$instance = array();
		$instance['title'] = ( ! empty( $new_instance['title'] ) ) ? wp_strip_all_tags( $new_instance['title'] ) : 'subsimple';
...
add_action( 'widgets_init', 'subsimple_register_widget' );  
function subsimple_register_widget() {  
    register_widget( 'subsimple_Recent_Posts' );  
}  
