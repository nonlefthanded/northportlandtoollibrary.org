...
function image_by_scan( $args = array() ) {
	...
	if ( isset( $matches ) && $matches[1][0] ) {
		...
		$height = $args['height'];
		$width = $args['width'];
		$timthumbprefix = get_bloginfo('template_url')."/inc/timthumb.php?src=";
...
function display_the_image( $args = array(), $image = false ) {
	...
	$html = '<img src="' . $image['url'] . '" alt="' . esc_attr( strip_tags( get_post_field( 'post_title', $post_id ) ) ) . '" class="' . $class . '" />';
