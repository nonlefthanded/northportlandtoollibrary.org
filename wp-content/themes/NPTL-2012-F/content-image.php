	...
	<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
		<header class="entry-header">
			...
			<hgroup>
				...
				<h2 class="entry-format"><?php _e( 'Image'); ?></h2>
                </div>
                
                <table width="484" border="0" align="center" cellpadding="0" cellspacing="0">
<tr>  <td width="92%" height="35" align="center">
<div class="date"><?php the_time('F') ?> </div><div class="date2"><?php the_time('j') ?> </div>
    ...
    <td width="24"><div class="date3"><?php the_time('Y') ?> </div></td>
			...
			<div class="comments-link">
				<!-- comments_popup_link( __( '<span class="leave-reply">Reply</span>'), __( '1'), __( '%') ); -->
			</div>
			<?php endif; ?>
		</header><!-- .entry-header -->
		...
		<footer class="entry-meta">
			...
			<div class="entry-meta">
				<?php
					printf( __( '<a href="%1$s" rel="bookmark"><time class="entry-date" datetime="%2$s" pubdate>%3$s</time></a> <span class="sep"> by </span> <span class="author vcard"><a class="url fn n" href="%4$s" title="%5$s">%6$s</a></span>'),
						get_permalink(),
						get_the_date( 'c' ),
						get_the_date(),
